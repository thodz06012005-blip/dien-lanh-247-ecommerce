    ...
    <div className="flex min-h-screen overflow-hidden bg-[#F4F7FB] text-slate-800 antialiased">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          <button type="button" className="absolute inset-0 bg-slate-950/70 backdrop-blur-sm" onClick={() => setMobileOpen(false)} aria-label="Đóng menu" />
          <aside className="sidebar-gradient absolute inset-y-0 left-0 flex w-[min(86vw,280px)] flex-col shadow-2xl">{sidebar}</aside>
        </div>
      )}
      ...
      <div className="flex h-screen min-w-0 flex-1 flex-col overflow-hidden">
        <header className="sticky top-0 z-20 flex h-16 shrink-0 items-center justify-between border-b border-slate-200/70 bg-white/95 px-4 shadow-sm backdrop-blur lg:px-7">
          ...
          <div className="flex items-center gap-2">
            <button type="button" onClick={() => setSearchOpen(true)} className="hidden min-h-10 items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs font-bold text-slate-500 hover:border-primary-200 hover:bg-white sm:flex">
              <Search className="h-4 w-4" /><span>Tìm module</span><kbd className="rounded-md border border-slate-200 bg-white px-1.5 py-0.5 text-[10px]">Ctrl K</kbd>
            </button>
            <button type="button" onClick={() => navigate('/')} className="relative flex h-10 w-10 items-center justify-center rounded-xl text-slate-500 hover:bg-slate-100" aria-label={`${attentionCount} công việc cần chú ý`}>
              <Bell className="h-[18px] w-[18px]" />
              {attentionCount > 0 && <span className="absolute right-1.5 top-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-red-500 px-1 text-[9px] font-black text-white">{Math.min(attentionCount, 9)}{attentionCount > 9 ? '+' : ''}</span>}
            </button>
            ...
            <div className="relative">
              <button type="button" onClick={() => setProfileOpen((value) => !value)} className="flex min-h-10 items-center gap-2 rounded-xl border border-transparent p-1.5 hover:border-slate-200 hover:bg-slate-50" aria-haspopup="menu" aria-expanded={profileOpen}>
                ...
                <div className="hidden text-left md:block"><strong className="block max-w-32 truncate text-xs text-slate-900">{admin?.name}</strong><span className="block text-[10px] font-bold uppercase tracking-wide text-slate-400">{admin?.role}</span></div>
                <ChevronDown className="hidden h-3.5 w-3.5 text-slate-400 md:block" />
              </button>
              {profileOpen && (
                <div role="menu" className="absolute right-0 mt-2 w-56 overflow-hidden rounded-2xl border border-slate-200 bg-white p-2 shadow-2xl">
                  <div className="border-b border-slate-100 px-3 py-3"><p className="truncate text-sm font-black text-slate-900">{admin?.name}</p><p className="mt-1 truncate text-xs text-slate-500">{admin?.email}</p></div>
                  <Link to="/profile" role="menuitem" className="mt-2 flex min-h-10 items-center gap-2 rounded-xl px-3 text-sm font-bold text-slate-600 hover:bg-slate-50"><UserRound className="h-4 w-4" />Hồ sơ và bảo mật</Link>
                  <button type="button" role="menuitem" onClick={handleLogout} className="flex min-h-10 w-full items-center gap-2 rounded-xl px-3 text-left text-sm font-bold text-red-600 hover:bg-red-50"><LogOut className="h-4 w-4" />Đăng xuất</button>
                </div>
              )}
            </div>
          </div>
      ...
      {searchOpen && (
        <div className="fixed inset-0 z-[70] flex items-start justify-center bg-slate-950/65 px-4 pt-[12vh] backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Tìm kiếm module quản trị">
          <button type="button" className="absolute inset-0" onClick={() => setSearchOpen(false)} aria-label="Đóng tìm kiếm" />
          <section className="relative w-full max-w-xl overflow-hidden rounded-[2rem] border border-white/10 bg-white shadow-2xl">
            <div className="flex items-center gap-3 border-b border-slate-100 px-5"><Command className="h-5 w-5 text-primary-600" /><input autoFocus value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} className="min-h-16 flex-1 text-base font-bold outline-none placeholder:text-slate-400" placeholder="Tìm đơn hàng, sản phẩm, dịch vụ..." /><button type="button" onClick={() => setSearchOpen(false)} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100" aria-label="Đóng"><X className="h-4 w-4" /></button></div>
            <div className="max-h-[50vh] overflow-y-auto p-3">
              {allowedSearchItems.map((item) => { const Icon = item.icon; return <button key={item.path} type="button" onClick={() => navigate(item.path)} className="flex w-full items-center gap-3 rounded-xl p-3 text-left hover:bg-slate-50"><div className="rounded-xl bg-blue-50 p-2 text-primary-600"><Icon className="h-4 w-4" /></div><div><strong className="text-sm text-slate-900">{item.label}</strong><p className="mt-0.5 text-xs text-slate-400">{item.path}</p></div></button>; })}
              {!allowedSearchItems.length && <div className="p-8 text-center text-sm text-slate-500">Không có module phù hợp trong phạm vi quyền hiện tại.</div>}
            </div>
          </section>
        </div>
      )}

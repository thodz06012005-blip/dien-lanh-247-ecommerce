...
export default function CmsPreviewModal({ type, id, onClose }: CmsPreviewModalProps) {
  ...
  const query = useQuery({
    queryKey: ['cms-preview', type, id],
    queryFn: async () => (await previewCms(type, id)).data,
  });

  const item = query.data;
  const cover = item?.coverUrl || item?.desktopMediaUrl || item?.url;
    ...
    <div className="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/70 p-3 backdrop-blur-sm sm:p-6" role="dialog" aria-modal="true" aria-label={`Preview ${meta.singular}`}>
      ...
      <section className="relative flex max-h-[94vh] w-full max-w-6xl flex-col overflow-hidden rounded-[2rem] bg-white shadow-2xl">
        <header className="flex shrink-0 items-center justify-between border-b border-slate-100 px-5 py-4 sm:px-7">
          <div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-primary-600">Draft preview · {meta.label}</p><h2 className="mt-1 text-xl font-black text-slate-950">{item ? cmsRecordLabel(item) : 'Đang tải preview...'}</h2></div>
          <button type="button" onClick={onClose} className="flex h-10 w-10 items-center justify-center rounded-xl text-slate-400 hover:bg-slate-100" aria-label="Đóng"><X className="h-5 w-5" /></button>
        </header>
          ...
          {item && (
            <div className="mx-auto max-w-5xl space-y-5">
              <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
                {cover ? <img src={resolveMedia(cover)} alt={String(item.altText || item.title || item.name || '')} className="max-h-[420px] w-full object-cover" /> : <div className="flex h-44 items-center justify-center bg-slate-100 text-slate-400"><ImageIcon className="h-12 w-12" /></div>}
                <div className="p-6 sm:p-8"><div className="flex flex-wrap gap-2"><span className="rounded-full bg-blue-50 px-3 py-1 text-[10px] font-black text-blue-700">{String(item.status || (item.isActive ? 'ACTIVE' : 'INACTIVE'))}</span>{item.isFeatured && <span className="rounded-full bg-amber-50 px-3 py-1 text-[10px] font-black text-amber-700">NỔI BẬT</span>}<span className="rounded-full bg-slate-100 px-3 py-1 text-[10px] font-black text-slate-600">VERSION {String(item.version || 1)}</span></div><h1 className="mt-4 text-3xl font-black tracking-tight text-slate-950">{cmsRecordLabel(item)}</h1>{item.excerpt && <p className="mt-4 text-base leading-8 text-slate-600">{String(item.excerpt)}</p>}</div>
              </section>

              {item.content ? <iframe title="Nội dung preview" sandbox="" srcDoc={previewHtml(item.content)} className="min-h-[520px] w-full rounded-[2rem] border border-slate-200 bg-white shadow-sm" /> : null}

              <section className="grid gap-4 rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm sm:grid-cols-2 sm:p-7">
                {Object.entries(item).filter(([key, value]) => !['content','snapshot','coverUrl','desktopMediaUrl','url'].includes(key) && value !== null && value !== undefined && typeof value !== 'object').slice(0, 18).map(([key, value]) => <div key={key} className="rounded-xl bg-slate-50 p-3"><span className="text-[10px] font-black uppercase tracking-wider text-slate-400">{key}</span><p className="mt-1 break-words text-sm font-semibold text-slate-700">{String(value)}</p></div>)}
              </section>

              {(item.ctaUrl || item.websiteUrl || item.canonicalUrl) && <div className="flex justify-end"><a href={String(item.ctaUrl || item.websiteUrl || item.canonicalUrl)} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-slate-950 px-5 text-sm font-black text-white">Mở liên kết <ExternalLink className="h-4 w-4" /></a></div>}
            </div>
          )}
        </div>
      </section>

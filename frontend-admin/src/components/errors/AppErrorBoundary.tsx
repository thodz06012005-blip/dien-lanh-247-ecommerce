...
> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    if (import.meta.env.DEV) {
      console.error('Unhandled admin application error', error, info.componentStack);
    }
  }
  ...
  render() {
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-slate-100 px-4 py-12">
        <section
          className="w-full max-w-lg rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-xl shadow-slate-300/40"
          role="alert"
          aria-live="assertive"
        >
          ...
          <h1 className="mt-5 text-2xl font-bold text-slate-900">Trang quản trị gặp sự cố</h1>
          <p className="mt-3 text-sm leading-6 text-slate-600">
            Thao tác hiện tại chưa hoàn tất. Hãy tải lại trang và kiểm tra lại dữ liệu trước khi thực
            hiện lần nữa.
          </p>

...
function statusTone(value: unknown) {
  ...
  if (['COMPLETED', 'CLOSED', 'ACTIVE', 'ACCEPTED', 'PAID'].includes(status)) return 'bg-emerald-50 text-emerald-700 border-emerald-100';
  if (['BREACHED', 'REJECTED', 'CANCELLED', 'OVERDUE'].includes(status)) return 'bg-red-50 text-red-700 border-red-100';
  if (['WAITING_PARTS', 'SENT', 'PARTIAL', 'WARRANTY'].includes(status)) return 'bg-amber-50 text-amber-700 border-amber-100';
...
function MetricCard({ label, value, icon: Icon, tone = 'blue' }: { label: string; value: ReactNode; icon: typeof Users; tone?: 'blue' | 'cyan' | 'amber' | 'emerald' | 'red' }) {
...
export default function Operations() {
  ...
  const [drawer, setDrawer] = useState<'device' | 'schedule' | 'dispatch' | 'quote' | 'payment' | 'completion' | 'warranty' | 'note' | 'reschedule' | 'sla' | null>(null);
  const [form, setForm] = useState<Record<string, string>>({});
  const [quoteLines, setQuoteLines] = useState([{ lineType: 'LABOR', description: '', quantity: '1', unit: 'lần', unitPrice: '0' }]);
  ...
  const workspace = useQuery({ queryKey: ['operations-workspace', activeRequestId], queryFn: () => getOperationsWorkspace(activeRequestId!), enabled: Boolean(activeRequestId) });
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      if (drawer === 'sla') return saveSlaPolicy({ serviceCategoryId: form.serviceCategoryId || undefined, priority: form.priority, responseMinutes: Number(form.responseMinutes), assignMinutes: Number(form.assignMinutes), arrivalMinutes: Number(form.arrivalMinutes), resolutionMinutes: Number(form.resolutionMinutes), warrantyResponseMinutes: Number(form.warrantyResponseMinutes), isActive: true });
      throw new Error('Thao tác chưa được hỗ trợ');
    },
  ...
  const openWorkspace = (id: string) => { setActiveRequestId(id); setWorkspaceId(id); setTab('workspace'); };
  const openDrawer = (value: typeof drawer, defaults: Record<string, string> = {}) => { setForm(defaults); setDrawer(value); mutation.reset(); };
  ...
  const customerColumns = useMemo<AdminDataColumn<AnyRow>[]>(() => [
    ...
    { key: 'action', header: '', render: (row) => <button onClick={() => { setForm({ userId: text(row.id), customerName: `${text(row.firstName)} ${text(row.lastName)}`.trim(), customerPhone: text(row.phone), customerEmail: text(row.email) }); setDrawer('device'); }} className="rounded-xl border border-blue-100 bg-blue-50 px-3 py-2 text-xs font-black text-blue-700">Thêm thiết bị</button> },
  ], []);
  ...
  const workspaceData: OperationsWorkspace | undefined = workspace.data;
...
function OperationForm({ type, form, setForm, technicians, quoteLines, setQuoteLines, error, saving, onSubmit }: { type: NonNullable<ReturnType<typeof useOperationType>>; form: Record<string, string>; setForm: (value: Record<string, string>) => void; technicians: AnyRow[]; quoteLines: Array<Record<string, string>>; setQuoteLines: (value: Array<Record<string, string>>) => void; error: string | null; saving: boolean; onSubmit: () => void }) {
  ...
  return <form onSubmit={(event) => { event.preventDefault(); onSubmit(); }} className="space-y-5">{type === 'device' && <div className="grid gap-4 sm:grid-cols-2">{field('customerName', 'Tên khách hàng', 'text', true)}{field('customerPhone', 'Số điện thoại', 'tel', true)}{field('customerEmail', 'Email', 'email')}{field('label', 'Tên gợi nhớ')}{field('applianceType', 'Loại thiết bị', 'text', true)}{field('brand', 'Thương hiệu')}{field('model', 'Model')}{field('serialNumber', 'Số serial')}{field('installationAddress', 'Địa chỉ lắp đặt')}{field('district', 'Khu vực')}{field('installedAt', 'Ngày lắp đặt', 'date')}{field('warrantyUntil', 'Bảo hành đến', 'date')}</div>}{type === 'schedule' && <div className="grid gap-4 sm:grid-cols-2"><label className="block sm:col-span-2"><span className="mb-1.5 block text-xs font-black text-slate-700">Kỹ thuật viên *</span><select value={form.technicianId ?? ''} onChange={(event) => setForm({ ...form, technicianId: event.target.value })} className={inputClass} required><option value="">Chọn kỹ thuật viên</option>{technicians.map((item) => <option key={text(item.id)} value={text(item.id)}>{text(item.name)} · {text(item.status)}</option>)}</select></label>{field('startAt', 'Bắt đầu', 'datetime-local', true)}{field('endAt', 'Kết thúc', 'datetime-local', true)}{field('note', 'Ghi chú')}</div>}{type === 'dispatch' && <div className="grid gap-4 sm:grid-cols-2"><label className="block sm:col-span-2"><span className="mb-1.5 block text-xs font-black text-slate-700">Kỹ thuật viên *</span><select value={form.technicianId ?? ''} onChange={(event) => setForm({ ...form, technicianId: event.target.value })} className={inputClass} required><option value="">Chọn kỹ thuật viên</option>{technicians.map((item) => <option key={text(item.id)} value={text(item.id)}>{text(item.name)} · {text(item.status)} · {num(item.rating).toFixed(1)}★</option>)}</select></label>{field('scheduledStart', 'Bắt đầu', 'datetime-local', true)}{field('scheduledEnd', 'Kết thúc', 'datetime-local', true)}<label className="sm:col-span-2"><span className="mb-1.5 block text-xs font-black text-slate-700">Lý do / ghi chú</span><textarea value={form.reason ?? ''} onChange={(event) => setForm({ ...form, reason: event.target.value })} className={`${inputClass} min-h-24 py-3`} /></label><label className="flex items-center gap-2 text-sm font-semibold text-slate-600 sm:col-span-2"><input type="checkbox" checked={form.force === 'true'} onChange={(event) => setForm({ ...form, force: String(event.target.checked) })} />Cho phép ngoại lệ chuyên môn/khu vực (được audit)</label></div>}{type === 'reschedule' && <div className="grid gap-4 sm:grid-cols-2">{field('scheduledStart', 'Lịch bắt đầu mới', 'datetime-local', true)}{field('scheduledEnd', 'Lịch kết thúc mới', 'datetime-local', true)}<label className="sm:col-span-2"><span className="mb-1.5 block text-xs font-black text-slate-700">Lý do *</span><textarea value={form.reason ?? ''} onChange={(event) => setForm({ ...form, reason: event.target.value })} className={`${inputClass} min-h-24 py-3`} required /></label></div>}{type === 'note' && <div><label><span className="mb-1.5 block text-xs font-black text-slate-700">Nội dung ghi chú *</span><textarea value={form.body ?? ''} onChange={(event) => setForm({ ...form, body: event.target.value })} className={`${inputClass} min-h-40 py-3`} required /></label></div>}{type === 'quote' && <div className="space-y-4"><div className="space-y-3">{quoteLines.map((line, index) => <div key={index} className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 sm:grid-cols-[120px_1fr_90px_130px_40px]"><select value={line.lineType} onChange={(event) => setQuoteLines(quoteLines.map((item, i) => i === index ? { ...item, lineType: event.target.value } : item))} className={inputClass}><option value="LABOR">Tiền công</option><option value="MATERIAL">Vật tư</option></select><input value={line.description} onChange={(event) => setQuoteLines(quoteLines.map((item, i) => i === index ? { ...item, description: event.target.value } : item))} className={inputClass} placeholder="Mô tả" required /><input type="number" min="0.001" step="0.001" value={line.quantity} onChange={(event) => setQuoteLines(quoteLines.map((item, i) => i === index ? { ...item, quantity: event.target.value } : item))} className={inputClass} /><input type="number" min="0" step="1000" value={line.unitPrice} onChange={(event) => setQuoteLines(quoteLines.map((item, i) => i === index ? { ...item, unitPrice: event.target.value } : item))} className={inputClass} /><button type="button" onClick={() => setQuoteLines(quoteLines.filter((_, i) => i !== index))} className="flex h-11 w-10 items-center justify-center rounded-xl text-red-500 hover:bg-red-50" aria-label="Xóa dòng"><X className="h-4 w-4" /></button></div>)}</div><button type="button" onClick={() => setQuoteLines([...quoteLines, { lineType: 'MATERIAL', description: '', quantity: '1', unit: 'cái', unitPrice: '0' }])} className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5 text-xs font-black text-blue-700">+ Thêm dòng</button><div className="grid gap-4 sm:grid-cols-3"><label><span className="mb-1.5 block text-xs font-black">Loại giảm giá</span><select value={form.discountType ?? 'FIXED'} onChange={(event) => setForm({ ...form, discountType: event.target.value })} className={inputClass}><option value="FIXED">Số tiền</option><option value="PERCENT">Phần trăm</option></select></label>{field('discountValue', 'Giá trị giảm', 'number')}{field('taxRate', 'Thuế (%)', 'number')}</div>{field('validUntil', 'Hiệu lực đến', 'datetime-local')}<label><span className="mb-1.5 block text-xs font-black">Ghi chú</span><textarea value={form.notes ?? ''} onChange={(event) => setForm({ ...form, notes: event.target.value })} className={`${inputClass} min-h-24 py-3`} /></label></div>}{type === 'payment' && <div className="grid gap-4 sm:grid-cols-2">{field('quoteId', 'ID báo giá', 'number')}<label><span className="mb-1.5 block text-xs font-black">Phương thức *</span><select value={form.method ?? 'CASH'} onChange={(event) => setForm({ ...form, method: event.target.value })} className={inputClass}><option value="CASH">Tiền mặt</option><option value="BANK_TRANSFER">Chuyển khoản</option><option value="CARD">Thẻ</option><option value="VNPAY">VNPay</option><option value="MOMO">MoMo</option></select></label>{field('amount', 'Số tiền', 'number', true)}{field('transactionReference', 'Mã giao dịch')}{field('note', 'Ghi chú')}</div>}{type === 'completion' && <div className="space-y-4"><label><span className="mb-1.5 block text-xs font-black">Chẩn đoán *</span><textarea value={form.diagnosis ?? ''} onChange={(event) => setForm({ ...form, diagnosis: event.target.value })} className={`${inputClass} min-h-28 py-3`} required /></label><label><span className="mb-1.5 block text-xs font-black">Công việc đã thực hiện *</span><textarea value={form.workPerformed ?? ''} onChange={(event) => setForm({ ...form, workPerformed: event.target.value })} className={`${inputClass} min-h-32 py-3`} required /></label><label><span className="mb-1.5 block text-xs font-black">Khuyến nghị</span><textarea value={form.recommendations ?? ''} onChange={(event) => setForm({ ...form, recommendations: event.target.value })} className={`${inputClass} min-h-24 py-3`} /></label><div className="grid gap-4 sm:grid-cols-2">{field('customerName', 'Người xác nhận')}{field('completedAt', 'Thời điểm hoàn thành', 'datetime-local', true)}</div></div>}{type === 'warranty' && <div className="space-y-4"><div className="grid gap-4 sm:grid-cols-2">{field('completionReportId', 'ID biên bản', 'number')}{field('deviceId', 'ID thiết bị', 'number')}{field('startsAt', 'Bắt đầu', 'datetime-local', true)}{field('endsAt', 'Kết thúc', 'datetime-local', true)}</div><label><span className="mb-1.5 block text-xs font-black">Phạm vi bảo hành *</span><textarea value={form.coverage ?? ''} onChange={(event) => setForm({ ...form, coverage: event.target.value })} className={`${inputClass} min-h-28 py-3`} required /></label><label><span className="mb-1.5 block text-xs font-black">Điều khoản loại trừ</span><textarea value={form.exclusions ?? ''} onChange={(event) => setForm({ ...form, exclusions: event.target.value })} className={`${inputClass} min-h-24 py-3`} /></label></div>}{type === 'sla' && <div className="grid gap-4 sm:grid-cols-2"><label><span className="mb-1.5 block text-xs font-black">Mức ưu tiên *</span><select value={form.priority ?? 'medium'} onChange={(event) => setForm({ ...form, priority: event.target.value })} className={inputClass}><option value="low">Thấp</option><option value="medium">Trung bình</option><option value="high">Cao</option><option value="urgent">Khẩn cấp</option></select></label>{field('serviceCategoryId', 'Mã loại dịch vụ')}{field('responseMinutes', 'Phản hồi (phút)', 'number', true)}{field('assignMinutes', 'Phân công (phút)', 'number', true)}{field('arrivalMinutes', 'Có mặt (phút)', 'number', true)}{field('resolutionMinutes', 'Hoàn thành (phút)', 'number', true)}{field('warrantyResponseMinutes', 'Phản hồi bảo hành (phút)', 'number', true)}</div>}{error && <div className="rounded-xl border border-red-100 bg-red-50 p-3 text-sm font-semibold text-red-700">{error}</div>}<button type="submit" disabled={saving} className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-500 px-5 text-sm font-black text-white shadow-lg disabled:opacity-50">{saving ? <RefreshCw className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}{saving ? 'Đang lưu...' : 'Xác nhận và lưu'}</button></form>;
}

function useOperationType() { return 'device' as const; }

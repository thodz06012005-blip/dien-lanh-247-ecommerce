...
test('Phase 13 sitemap is generated from paginated published APIs', () => {
  const packageJson = read('frontend-user/package.json');
  const generator = read('frontend-user/scripts/generate-sitemap.mjs');
  assert.match(packageJson, /"prebuild": "npm run sitemap:generate"/);
  for (const endpoint of ['/services', '/projects', '/posts', '/products']) {
    assert.match(generator, new RegExp(endpoint));
  }
  assert.match(generator, /totalPages/);
  assert.match(generator, /SITEMAP_API_URL/);
});
...
test('Phase 13 measures mobile routes, desktop home and field web vitals', () => {
  ...
  for (const route of ['home / mobile', 'services /services mobile', 'products /products mobile', 'articles /articles mobile', 'booking /service-booking mobile', 'home / desktop']) {

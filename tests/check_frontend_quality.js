...
const routeChunks = jsFiles.filter(item => /^(Home|ServiceBooking|TechnicianPortal|Login|MyServices|TrackService)-/.test(item.file));
const retiredCommerceChunks = jsFiles.filter(item => /^(Products|ProductDetail|Cart|Checkout|Orders)-/.test(item.file));
...
if (retiredCommerceChunks.length) throw new Error(`Retired commerce chunks found: ${retiredCommerceChunks.map(item => item.file).join(', ')}`);

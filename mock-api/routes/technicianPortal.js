...
router.patch('/technician/jobs/:id/complete', auth, (req, res) => {
  ...
  const now = new Date().toISOString(); job.status = 'completed'; job.finalPrice = finalPrice; job.completionNote = completionNote; job.completionPhotos = photos; job.completedAt = now; job.updatedAt = now; job.paymentStatus = job.paymentStatus || 'unpaid'; if (!job.statusHistory) job.statusHistory = []; job.statusHistory.push({ status: 'completed', note: completionNote, updatedBy: 'technician', createdAt: now }); log(job, 'Kỹ thuật viên hoàn thành công việc', tech.name, completionNote); tech.status = (db.serviceRequests || []).some(item => item.id !== job.id && item.assignedTechnicianId === tech.id && ['assigned', 'in_progress'].includes(item.status)) ? 'busy' : 'available'; tech.completedCount = Number(tech.completedCount || 0) + 1; writeDB(db); return respondSuccess(res, job, 'Đã hoàn thành công việc');
});
router.get('/technician/earnings', auth, (req, res) => { const db = readDB(); const config = db.settings?.businessConfig?.finance || {}; const completed = (db.serviceRequests || []).filter(item => item.assignedTechnicianId === req.technicianId && item.status === 'completed'); const rate = Number(config.technicianPayRate || 40); const items = completed.map(job => ({ id: job.id, completedAt: job.completedAt || job.updatedAt, revenue: Number(job.finalPrice || 0), earning: config.technicianPayType === 'fixed' ? rate : Math.round(Number(job.finalPrice || 0) * rate / 100) })); return respondSuccess(res, { total: items.reduce((sum, item) => sum + item.earning, 0), thisMonth: items.filter(item => item.completedAt?.startsWith(new Date().toISOString().slice(0, 7))).reduce((sum, item) => sum + item.earning, 0), jobs: items }); });

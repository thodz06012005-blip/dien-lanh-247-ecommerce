...
export class AuthController {
  ...
  async register(
    ...
    const { tokens, user, verification } = await this.authService.register(registerDto, req);
    this.setTokenCookies(res, tokens);
    return {
      success: true,
      message: 'Đăng ký thành công',
      data: { user, verificationSent: verification.verificationSent, debugToken: verification.debugToken },
    };
  }
  ...
  async login(
    ...
    const { tokens, user, linkedCount } = await this.authService.login(loginDto, req);
    this.setTokenCookies(res, tokens);
    return {
      success: true,
  ...
  async logoutAll(
    ...
    await this.authService.revokeAllSessions(user.userId, 'CUSTOMER_LOGOUT_ALL');
    this.clearTokenCookies(res);
  ...
  async resetPassword(
    ...
    this.clearTokenCookies(res);
    return { success: true, message: 'Mật khẩu đã được thay đổi. Vui lòng đăng nhập lại.' };
  }
  ...
  async verifyEmail(@Body() dto: VerifyEmailDto) {
    ...
    return { success: true, message: 'Xác minh email thành công', data: result };
  }
  ...
  async resendVerification(@CurrentUser() user: CurrentSessionUser) {
    ...
    return {
      success: true,
      message: result.verificationSent ? 'Đã gửi lại email xác minh' : 'Email đã được xác minh',
      data: result.debugToken ? { debugToken: result.debugToken } : undefined,
    };
  }
  ...
  private clearTokenCookies(res: Response) {
    ...
    res.clearCookie('refreshToken', { httpOnly: true, secure, sameSite, path: '/api/v1/auth/refresh' });
  }
}

...
function readEnum<T extends string>(
  ...
  const value = readString(source, name, { fallback }) as T;
...
function assertStrongSecret(name: string, value: string) {
  if (value.length < 32) {
    throw new Error(
      `${name} must contain at least 32 characters in staging and production.`,
    );
  }
...
export function validateEnvironment(
  ...
  const nodeEnvironment = readString(source, 'NODE_ENV', {
  ...
  const jwtAccessSecret = readString(source, 'JWT_ACCESS_SECRET', {
    required: true,
  });
  const jwtRefreshSecret = readString(source, 'JWT_REFRESH_SECRET', {
    required: true,
  });
  ...
  const enableDevEndpoints = readBoolean(
    source,
    'ENABLE_DEV_ENDPOINTS',
    nodeEnvironment === 'development',
  );
  const enableDemoAccounts = readBoolean(
    source,
  ...
  if (nodeEnvironment === 'staging' || nodeEnvironment === 'production') {
    ...
    assertStrongSecret('AUDIT_LOG_HASH_SALT', auditHashSalt);
    ...
    if (enableDevEndpoints || enableDemoAccounts) {
      throw new Error(
        'Development endpoints and demo accounts must be disabled outside development.',
      );
    }
    if (!cookieSecure) {
      throw new Error(
  ...
  return {
    ...
    API_PREFIX: readString(source, 'API_PREFIX', { fallback: 'api/v1' }).replace(
      /^\/+|\/+$/g,
      '',
    ),
    ...
    BCRYPT_SALT_ROUNDS: readInteger(
      source,
      'BCRYPT_SALT_ROUNDS',
      10,
      8,
    ...
    LOGIN_RATE_LIMIT_WINDOW_MS: readInteger(
      source,
    ...
    LOGIN_RATE_LIMIT_MAX_PER_EMAIL: readInteger(
      source,
    ...
    LOGIN_RATE_LIMIT_MAX_PER_IP: readInteger(
      source,
    ...
    LOGIN_RATE_LIMIT_MAX_PER_IP_EMAIL: readInteger(
      source,
      'LOGIN_RATE_LIMIT_MAX_PER_IP_EMAIL',
      5,
      1,
      100,
    ),
    LOGIN_RATE_LIMIT_LOCK_MS: readInteger(
      source,
    ...
    JSON_BODY_LIMIT: readString(source, 'JSON_BODY_LIMIT', {
      fallback: '1mb',
    }),
    URLENCODED_BODY_LIMIT: readString(source, 'URLENCODED_BODY_LIMIT', {
      fallback: '100kb',
    }),
    ...
    AUDIT_LOG_MAX_ENTRIES: readInteger(
      source,
    ...
    AUDIT_LOG_PATH: readString(source, 'AUDIT_LOG_PATH', {
      fallback: 'var/audit/security-audit.jsonl',
    }),
    AUDIT_LOG_HASH_SALT: auditHashSalt,
    BACKUP_DIRECTORY: readString(source, 'BACKUP_DIRECTORY', {
      fallback: 'var/backups',
    }),
    BACKUP_RETENTION_DAYS: readInteger(
      source,

      ...
      <header className="fixed inset-x-0 top-0 z-50 border-b border-slate-200/80 bg-white/95 shadow-sm backdrop-blur-xl">
        <div className="hidden bg-[#061527] text-white md:block">
          <div className="mx-auto flex h-9 max-w-7xl items-center justify-between px-6 text-xs lg:px-8">
            <div className="flex items-center gap-5">
              ...
              <a href={`https://zalo.me/${zalo.replace(/\s+/g, '')}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 font-bold text-slate-200 hover:text-white">
                <MessageCircle aria-hidden="true" className="h-3.5 w-3.5 text-cyan-300" /> Tư vấn qua Zalo
              </a>
            </div>
            <div className="flex items-center gap-4 text-slate-300">
              <Link to="/service-lookup" className="inline-flex items-center gap-1.5 hover:text-white"><Search aria-hidden="true" className="h-3.5 w-3.5" />Tra cứu yêu cầu</Link>
        ...
        <div className="mx-auto flex h-16 max-w-7xl items-center gap-4 px-4 sm:px-6 md:h-[76px] lg:px-8">
          <Link to="/" className="flex shrink-0 items-center gap-2.5">
            <span aria-hidden="true" className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-primary-600 to-cyan-500 text-lg font-black text-white shadow-lg shadow-blue-500/20">D</span>
            <span className="hidden text-base font-black leading-none tracking-tight text-slate-950 sm:block">
              Điện Lạnh <span className="text-primary-700">247</span>
              <span className="mt-1 block text-[10px] font-bold uppercase tracking-[0.16em] text-slate-600">Dịch vụ kỹ thuật</span>
            </span>
          </Link>

          <nav className="ml-auto hidden items-center gap-0.5 xl:flex" aria-label="Điều hướng chính">
            {navigation.map((item) => (
              <NavLink key={item.to} to={item.to} end={item.to === '/'} className={({ isActive }) => `rounded-lg px-3 py-2 text-sm font-extrabold transition ${isActive ? 'bg-blue-50 text-primary-800' : 'text-slate-700 hover:bg-slate-50 hover:text-slate-950'}`}>
                {item.label}
              </NavLink>
            ))}
          </nav>
        ...
        <div className="fixed inset-0 z-[60] xl:hidden" id="customer-mobile-menu">
          ...
          <aside className="absolute bottom-0 right-0 top-0 flex w-[min(88vw,380px)] flex-col overflow-y-auto bg-white shadow-2xl" aria-label="Menu di động">
            ...
            <nav className="grid gap-1 p-4" aria-label="Điều hướng di động">
              {navigation.map((item) => (
                <NavLink key={item.to} to={item.to} end={item.to === '/'} className={({ isActive }) => `flex min-h-12 items-center gap-3 rounded-xl px-4 text-base font-black ${isActive ? 'bg-blue-50 text-primary-800' : 'text-slate-700 hover:bg-slate-50'}`}>
                  {item.icon && <item.icon aria-hidden="true" className="h-4 w-4" />}{item.label}
                </NavLink>
              ))}
            </nav>

            <div className="mt-auto grid gap-3 border-t border-slate-200 p-4">
              <Link to="/service-booking" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-orange-700 px-4 text-sm font-black text-white"><CalendarDays aria-hidden="true" className="h-4 w-4" /> Đặt lịch kỹ thuật</Link>
              <Link to="/service-lookup" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-primary-200 bg-primary-50 px-4 text-sm font-black text-primary-800"><Search aria-hidden="true" className="h-4 w-4" /> Tra cứu yêu cầu</Link>
              ...
              {isAuthenticated && <button type="button" onClick={handleLogout} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl text-sm font-black text-red-700 hover:bg-red-50"><LogOut aria-hidden="true" className="h-4 w-4" /> Đăng xuất</button>}
            </div>
          </aside>
        </div>
      )}

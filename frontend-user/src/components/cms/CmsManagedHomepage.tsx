        ...
        <section className="border-y border-slate-100 bg-slate-50 py-12">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-black uppercase tracking-[0.16em] text-slate-700">Đối tác đồng hành</h2>
            </div>
            <div className="mt-7 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
              {bundle.partners.map((partner) => {
                const content = (
                  <div className="flex min-h-24 items-center justify-center rounded-2xl border border-slate-200 bg-white p-4 text-center shadow-sm">
                    {partner.logoUrl ? (
                      <img
                        src={resolveMedia(partner.logoUrl)}
                      ...
                      <span className="text-sm font-black text-slate-700">{partner.name}</span>
                    )}
                  </div>
                );
                return partner.websiteUrl ? (
                  <a key={partner.id} href={partner.websiteUrl} target="_blank" rel="noopener noreferrer" aria-label={`Mở website ${partner.name}`}>{content}</a>
                ) : (

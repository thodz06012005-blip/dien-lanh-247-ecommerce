...
interface BookingForm { customerName: string; customerPhone: string; customerAddress: string; district: string; areaId: string; applianceType: string; serviceCategoryId: string; issueDescription: string; preferredDate: string; preferredTimeSlot: string; note: string; }
interface MediaItem { name: string; type: string; dataUrl: string; size: number; }
const emptyForm: BookingForm = { customerName: '', customerPhone: '', customerAddress: '', district: '', areaId: '', applianceType: '', serviceCategoryId: '', issueDescription: '', preferredDate: '', preferredTimeSlot: '', note: '' };
...
export default function ServiceBooking() {
  ...
  const selectedArea = areas.find(item => item.id === form.areaId);
  useEffect(() => { if (!form.areaId && form.district) { const area = areas.find(item => item.name === form.district); if (area) setForm(prev => ({ ...prev, areaId: area.id })); } }, [areas, form.areaId, form.district]);
  ...
  const validateStep = (target: number) => {
    ...
    if (target === 2 && (!form.areaId || !form.customerAddress.trim() || !form.preferredDate || !form.preferredTimeSlot || !form.customerName.trim() || !form.customerPhone.trim())) return 'Vui lòng điền đầy đủ khu vực, lịch hẹn và thông tin liên hệ.';

...
export default function Account() {
  ...
  const activeTab: Tab = tabs.some((item) => item.id === requestedTab) ? requestedTab as Tab : 'overview';

  const [profile, setProfile] = useState({ firstName: '', lastName: '', phone: '' });
  const [addressForm, setAddressForm] = useState({
    label: 'Nhà riêng', fullName: '', phone: '', province: '', district: '', ward: '',
  ...
  useEffect(() => {
    ...
    setAddressForm((value) => ({
      ...value,
      fullName: `${current.lastName || ''} ${current.firstName || ''}`.trim(),
      phone: current.phone || '',
    }));
    setClaim((value) => ({ ...value, phone: current.phone || '' }));
  ...
  const addressMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setAddressForm((value) => ({ ...value, province: '', district: '', ward: '', streetAddress: '', note: '', isDefault: false }));
    },
    onError: (error) => showError(getApiErrorMessage(error)),
  });
  ...
  const revokeMutation = useMutation({
    ...
    onSuccess: (session) => {
      ...
      showSuccess('Phiên đăng nhập đã được thu hồi.');
    },
    onError: (error) => showError(getApiErrorMessage(error)),
  });
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-9 sm:px-6 lg:px-8">
          ...
          <div className="mt-6 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="flex items-center gap-5">
              <div className="flex h-16 w-16 items-center justify-center rounded-3xl bg-gradient-to-br from-cyan-400 to-blue-600 text-2xl font-black shadow-xl shadow-cyan-500/20">{(accountUser.firstName || accountUser.email).charAt(0).toUpperCase()}</div>
              <div><p className="text-xs font-black uppercase tracking-[0.22em] text-cyan-300">Customer Hub</p><h1 className="mt-2 text-3xl font-black">{accountUser.lastName} {accountUser.firstName}</h1><p className="mt-1 text-sm text-slate-400">{accountUser.email}</p></div>
            </div>
        ...
        <aside className="h-fit rounded-[2rem] border border-slate-200 bg-white p-3 shadow-sm lg:sticky lg:top-32">
          <nav className="space-y-1" aria-label="Khu vực tài khoản">
            {tabs.map(({ id, label, icon: Icon }) => (
              <button key={id} type="button" onClick={() => setSearchParams({ tab: id })} className={`flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-left text-sm font-black transition ${activeTab === id ? 'bg-primary-600 text-white shadow-lg shadow-primary-500/20' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-950'}`}>
                <Icon className="h-4 w-4" /><span className="flex-1">{label}</span>{id === 'notifications' && unread > 0 && <span className="rounded-full bg-orange-500 px-2 py-0.5 text-[10px] text-white">{unread}</span>}<ChevronRight className="h-4 w-4 opacity-50" />
              </button>
            ))}

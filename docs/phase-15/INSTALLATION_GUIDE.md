...
Bốn container ứng dụng không công khai cổng trực tiếp. Chỉ gateway mở cổng 80/443; database và backend nằm trong mạng Docker riêng. Root credential của MySQL chỉ được đưa vào container database, không được đưa vào backend.
...
cp deploy/env/database.env.example deploy/env/database.env
cp deploy/env/backend.env.example deploy/env/backend.env
chmod 600 deploy/env/production.env deploy/env/database.env deploy/env/backend.env
```

Ba file có trách nhiệm tách biệt:

| File | Container nhận | Nội dung |
|---|---|---|
| `production.env` | Docker Compose interpolation | domain và version, không có credential |
| `database.env` | chỉ MySQL | database name, app password và root password |
| `backend.env` | chỉ NestJS | app database URL, JWT/audit secret, seed và integrations |

Thay mọi giá trị dạng `<...>` bằng secret thật. `NODE_ENV=production` sẽ từ chối secret ngắn, placeholder, cookie không Secure hoặc CORS wildcard. Backend chỉ dùng user database có quyền trên schema ứng dụng; không dùng root.
...
Đặt `RUN_SEED=true` trong `backend.env` đúng một lần để tạo dữ liệu nền và tài khoản admin; sau khi hoàn tất phải đổi về `false`.
